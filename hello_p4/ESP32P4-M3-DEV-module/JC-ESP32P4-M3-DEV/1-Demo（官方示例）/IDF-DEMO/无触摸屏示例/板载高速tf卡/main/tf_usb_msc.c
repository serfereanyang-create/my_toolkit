...
void app_main(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "bsp_sdcard_mount failed: %s (0x%x)", esp_err_to_name(err), err);
        ESP_LOGE(TAG, "check TF card insertion, FAT filesystem, and that the High speed USB port is used");
        return;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "tinyusb_msc_new_storage_sdmmc failed: %s (0x%x)", esp_err_to_name(err), err);
        return;
    }

    err = tinyusb_msc_get_storage_capacity(s_storage_hdl, &sector_count);
    if (err == ESP_OK) {
        ESP_ERROR_CHECK(tinyusb_msc_get_storage_sector_size(s_storage_hdl, &sector_size));
        ESP_LOGI(TAG, "USB MSC storage ready: sector_count=%"PRIu32", sector_size=%"PRIu32,
                 sector_count, sector_size);
    } else {
        ESP_LOGW(TAG, "unable to query MSC storage capacity: %s (0x%x)", esp_err_to_name(err), err);
    }
